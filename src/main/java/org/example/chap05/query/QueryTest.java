package org.example.chap05.query;
...
public class QueryTest {
    ...
    private static void exucuteQuery(Connection connection) throws SQLException {
        String author = select("Authors:",authors);
        String publisher = select("Publishers:",publishers);
        PreparedStatement stat;
        if (!author.equals("Any") && !publisher.equals("Any")) {
            stat = connection.prepareStatement(authorPublisherQuery);
            stat.setString(1,author);
            stat.setString(2,publisher);
        } else if (!author.equals("Any") && publisher.equals("Any")) {
            stat = connection.prepareStatement(authorQuery);
            stat.setString(1,author);
        } else if (author.equals("Any") && !publisher.equals("Any")) {
            stat = connection.prepareStatement(publisherQuery);
            stat.setString(1,publisher);
        } else {
            stat = connection.prepareStatement(allQuery);
        }
